...
const SELECT_FIELDS = `
  ...
  to_char(joined_at, 'YYYY-MM-DD') AS "joinedAt", connections, session_id AS "sessionId"
`;
...
/**
 * Claims a fresh session for this driver — call on every successful
 * login. Whatever session ID was issued to a previous device stops
 * matching the one now stored here, so that device gets signed out the
 * next time it loads a page (see the check in app/dashboard/layout.js).
 * Enforces one active login at a time per driver.
 */
export async function setDriverSessionId(id, sessionId) {
  const { rows } = await query(
    `UPDATE drivers SET session_id = $2 WHERE id = $1 RETURNING ${SELECT_FIELDS}`,
    [id, sessionId]
  );
  return rowToDriver(rows[0]);
}

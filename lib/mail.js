...
export async function sendMail({ to, subject, heading, paragraphs = [], button }) {
  ...
  <div style="font-family:Inter,Arial,sans-serif;max-width:560px;margin:0 auto;color:#141a25;">
    <div style="padding:22px 0 10px;display:flex;align-items:center;gap:10px;font-weight:900;font-size:22px;letter-spacing:-.03em;"><img src="${site}/logo-mark-192.png" width="34" height="34" alt="" style="display:inline-block;vertical-align:middle;border-radius:9px;margin-right:10px;"><span style="vertical-align:middle;">TAXI<span style="color:#56cd7b;">CHARG</span></span></div>
    ...
    ${button ? `<p style="margin:20px 0;"><a href="${esc(button.url)}" style="display:inline-block;background:#56cd7b;color:#fff;text-decoration:none;font-weight:700;padding:12px 20px;border-radius:10px;">${esc(button.label)}</a></p><p style="font-size:12px;color:#4a5a68;">If the button does not work, copy this link: ${esc(button.url)}</p>` : ""}
